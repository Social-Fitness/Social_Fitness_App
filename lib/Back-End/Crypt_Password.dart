...
String decryptAESCryptoJS(String encrypted, String passphrase) {
  try {
    Uint8List encryptedButesWithSalt=base64.decode(encrypted);
    Uint8List encryptedBytes=encryptedButesWithSalt.sublist(16, encryptedButesWithSalt.length);
    final salt=encryptedButesWithSalt.sublist(8,16);
    var keyndIV=deriveKeyAndIV(passphrase, salt);
    final key=encrypt.Key(keyndIV.item1);
    final iv=encrypt.IV(keyndIV.item2);
    final encrypter=encrypt.Encrypter(encrypt.AES(key, mode:encrypt.AESMode.cbc,padding: "PKCS7"));
...
Tuple2<Uint8List, Uint8List> deriveKeyAndIV(String passphrase, Uint8List salt) {
  ...
  while(!enoughBytesForKey) {
    int preHashLenght=currentHash.length+password.length+salt.length;
